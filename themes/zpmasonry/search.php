			...
			<div id="mason">
				...
				<div class="box <?php echo $zpmas_col_image; ?>">
					...
					<div class="image-block" style="width:<?php echo $zpmas_image_size; ?>px;height:<?php echo $zpmas_image_size; ?>px;">
						...
						<?php if (!isImageVideo()) { ?>
						<div class="overlay">
							<a class="zpmas-cb" href="<?php if ($zpmas_cbtarget) { echo htmlspecialchars(getDefaultSizedImage()); } else { echo htmlspecialchars(getUnprotectedImageURL()); } ?>" title="<?php echo getBareImageTitle();?>"><img src="<?php echo $_zp_themeroot; ?>/images/full-screen<?php if ($zpmas_css == 'dark') echo "-inv"; ?>.png" alt="<?php echo gettext('Preview'); ?>" /></a>
						</div>
						<?php } ?>
					</div>
				</div>
