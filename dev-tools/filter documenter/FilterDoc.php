...
function processFilters() {
		...
		if (count($params[0])) {
			...
			foreach ($params[0] as $script) {
				if (!$class) {
					...
					if (!array_key_exists('*'.$class.'_'.$subclass, $filterDescriptions)) {
						$filterDescriptions['*'.$class.'_'.$subclass] = '';
					}
				}
	...
	//todo: sort by the description to make finding new ones easier
	ksort($filterDescriptions);
	...
	foreach ($newfilterlist as $filter) {
		...
		if ($subclass !== $filter['subclass']) {	// new subclass
			...
			if (array_key_exists('*'.$class.'_'.$subclass, $filterDescriptions)) {
				$subclasshead = $filterDescriptions['*'.$class.'_'.$subclass];
			} else {
				$subclasshead = '';
			}
