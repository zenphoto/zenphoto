...
if ($result['ValidJpeg'] == 1) {
	...
	$header = fread( $in, 6 ); // Exif block starts with 'Exif' header
	if ($header == "Exif\0\0") {  // EXIF or XMP Marker ?
		$result['ValidEXIFData'] = 1;
	} else {
		$data = fread( $in, hexdec($size)-2-6); // skip XMP data
		$globalOffset+=hexdec($size)+2;
		$data = bin2hex(fread( $in, 2 ));       // try next Marker
		$size = bin2hex(fread( $in, 2 ));
		$header = fread( $in, 6 ); // Exif block starts with 'Exif' header
		if ($data == 'ffe1' && $header == "Exif\0\0") {
			$result['ValidEXIFData'] = 1;
		} else {
			fclose($in);
			fclose($seek);
