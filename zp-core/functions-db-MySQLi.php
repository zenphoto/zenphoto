...
function db_connect($config, $errorstop = true) {
	...
	if (!$_zp_DB_connection) {
		if ($errorstop) {
			zp_error(gettext('MySQLi Error: ZenPhotoGraphics could not instantiate a connection.'));
		}
		return false;
	}
	...
	if (!$_zp_DB_connection->select_db($config['mysql_database'])) {
		if ($errorstop) {
			zp_error(sprintf(gettext('MySQLi Error: MySQLi returned the error %1$s when ZenPhotoGraphics tried to select the database %2$s.'), $_zp_DB_connection->error, $config['mysql_database']));
		}
		return false;
	}
