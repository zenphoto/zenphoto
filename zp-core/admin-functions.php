...
function printAdminHeader($tab, $subtab = NULL) {
	 ...
	 */
	function generateUnorderedListFromArray($currentValue, $list, $prefix, $alterrights, $sort, $localize, $class = NULL, $extra = NULL, $postArray = 1) {
		if (is_null($extra))
			$extra = array();
		if (!empty($class))
			$class = ' class="' . $class . '" ';
...
function processManagedObjects($i, &$rights) {
	...
	if (isset($_POST['user'][$i]['managed'])) {
		...
		foreach (array('albums', 'pages', 'news') as $class) {
