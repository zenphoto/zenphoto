...
// force UTF-8 Ø
/**
 * Support class for internationalization
 * 
 * Note the file definitions-i18n.php must be included before this class
 * 
 * @since 1.7  Reworked as class replacing functions-i18n.php
 * 
...
class i18n {
	...
	/**
	 * Stores all active languages (translation catalogues)
	 * @since 1.7 Replaces the global $_zp_active_languages. Use i18n::generateLanguageList(false) instead
	 * @var array|null
	 */
	public static $active_languages = null;

	/**
	 * Stores all languages (translation catalogues)
	 * @since 1.7 Replaces the global $_zp_all_languages. Use i18n::generateLanguageList(false) instead
	 * @var array|null
	 */
	public static $all_languages = null;

	/**
	 * Holds the current locale
	 * @since 1.7 Replaces the global $_zp_current_locale. Use i18n::getUserLocale() instead
	 * @var string
	 */
	public static $current_locale = null;

	/**
	 * Stores the array of subdomains if set, otherwise null
	 * @since 1.7 Replaces the global $_zp_locale_subdomains Use i18n::getLocaleSubdomains() instead
	 * @var array|null
	 */
	public static $locale_subdomains = null;

	/**
	 * Gets an array of all language locales and translated names
	 * 
	...
	static function generateLanguageList($all = false) {
		if (is_null(self::$all_languages)) {
			$zp_languages = self::getLanguageArray();
			$dir = @opendir(SERVERPATH . "/" . ZENFOLDER . "/locale/");
			self::$active_languages = self::$all_languages = array();
			if ($dir !== false) {
				while ($dirname = readdir($dir)) {
					if (is_dir(SERVERPATH . "/" . ZENFOLDER . "/locale/" . $dirname) && (substr($dirname, 0, 1) != '.')) {
						...
						self::$all_languages[$language] = $dirname;
						if (!getOption('disallow_' . $dirname)) {
							self::$active_languages[$language] = $dirname;
						}
					}
				}
				closedir($dir);
			}
			ksort(self::$all_languages);
			ksort(self::$active_languages);
		}
		if ($all) {
	...
	static function setMainDomain() {
		...
		return self::setupCurrentLocale($locale);
	}
	 ...
	 * @return string
	 */
	static function gettext_th($string, $theme = Null) {
		global $_zp_gallery;
		if (empty($theme)) {
			$theme = $_zp_gallery->getCurrentTheme();
		}
		self::setupDomain($theme, 'theme');
	 ...
	 * @return string
	 */
	static function ngettext_th($msgid1, $msgid2, $n, $theme = NULL) {
		global $_zp_gallery;
		if (empty($theme)) {
			$theme = $_zp_gallery->getCurrentTheme();
		}
	 ...
	 * @return string
	 */
	static function gettext_pl($string, $plugin) {
		self::setupDomain($plugin, 'plugin');
	 ...
	 * @return string
	 */
	static function ngettext_pl($msgid1, $msgid2, $n, $plugin) {
		self::setupDomain($plugin, 'plugin');
	...
	static function setLocale($locale) {
		...
		$locale_hyphen = self::getLangAttributeLocale($locale);
	...
	static function setupDomain($domain = NULL, $type = NULL) {
		...
		self::$active_languages = self::$all_languages = NULL;
	}
	...
	static function validateLocale($userlocale, $source) {
		...
		$languageSupport = i18n::generateLanguageList();
		$locale = NULL;
		if (!empty($userlocale)) {
		...
		if (!self::$current_locale) {
			...
			if (isset($_REQUEST['locale'])) {
				if (isset($_POST['locale'])) {
					self::$current_locale = self::validateLocale(sanitize($_POST['locale']), 'POST');
				} else {
					self::$current_locale = self::validateLocale(sanitize($_GET['locale']), 'URI string');
				}
				if (self::$current_locale) {
					zp_setCookie('zpcms_locale', self::$current_locale);
				}
				if (DEBUG_LOCALE) {
					debugLog("dynamic_locale from URL: " . sanitize($_REQUEST['locale']) . "=> " . self::$_current_locale);
				}
			} else {
				$matches = explode('.', @$_SERVER['HTTP_HOST']);
				if (self::$current_locale = self::validateLocale($matches[0], 'HTTP_HOST')) {
					zp_clearCookie('zpcms_locale');
				}
			}

			if (!self::$current_locale && is_object($_zp_current_admin_obj)) {
				self::$current_locale = $_zp_current_admin_obj->getLanguage();
				if (DEBUG_LOCALE) {
					debugLog("locale from user: " . self::$current_locale);
				}
			}

			if (!self::$current_locale) {
				$localeOption = getOption('locale');
				self::$current_locale = zp_getCookie('zpcms_locale');
				if (DEBUG_LOCALE) {
					debugLog("locale from option: " . $localeOption . '; dynamic locale=' . self::$current_locale);
				}
				if (empty($localeOption) && empty(self::$current_locale)) { // if one is not set, see if there is a match from 'HTTP_ACCEPT_LANGUAGE'
					$languageSupport = i18n::generateLanguageList();
					$userLang = self::parseHttpAcceptLanguage();
					foreach ($userLang as $lang) {
						$l = strtoupper($lang['fullcode']);
						self::$current_locale = self::validateLocale($l, 'HTTP Accept Language');
						if (self::$current_locale) {
							break;
						}
					}
				} else {
					if (empty(self::$current_locale)) {
						self::$current_locale = $localeOption;
					}
				}
			}
			...
			if (empty(self::$current_locale)) {
				...
				if (in_array('en_US', $languageSupport)) {
					...
					self::$current_locale = array_shift($languageSupport);
				}
			} else {
				setOption('locale', self::$current_locale, false);
			}
			if (DEBUG_LOCALE) {
				debugLog("getUserLocale Returning locale: " . self::$current_locale);
			}
		}
	 ...
	 * @return string
	 */
	static function getLanguageString($dbstring, $locale = NULL) {
	 ...
	 * @return int
	 */
	static function timezoneDiff($server, $local) {
		if (function_exists('timezone_abbreviations_list')) {
			$timezones = timezone_abbreviations_list();
			foreach ($timezones as $key => $zones) {
				foreach ($zones as $id => $zone) {
	 ...
	 */
	static function getAllTranslations($text) {
		$entry_locale = self::getUserLocale();
		$result = array('en_US' => $text);
		$languages = i18n::generateLanguageList();
		foreach ($languages as $language) {
			self::setupCurrentLocale($language);
			$xlated = gettext($text);
			if ($xlated != $text) { // the string has a translation in this language
				$result[$language] = $xlated;
			}
		}
		self::setupCurrentLocale($entry_locale);
		if (count($result) == 1) {
			return $text;
		}
	 ...
	 * @return string
	 */
	static function getLangAttributeLocale($locale = NULL) {
		if (empty($locale)) {
			$locale = self::getUserLocale();
		}
	 ...
	 */
	static function printLangAttribute($locale = null) {
		echo ' lang="' . sanitize(self::getLangAttributeLocale($locale)) . '"';
	}
	 ...
	 * @return string
	 */
	static function getFormattedLocaleDate($format = 'Y-m-dd', $datetime = '') {
		$locale = self::getUserLocale();
	 ...
	 * 
	 */
	static function getLanguageSubdomains() {
		if (!is_null(self::$locale_subdomains)) {
			return self::$locale_subdomains;
		}
		$domains = array();
		...
		// cache in new $locale_subdomains later
		return self::$locale_subdomains = $langs;
	}
	 ...
	 * @return array
	 */
	static function getSystemLocales($plainarray = false) {
	 ...
	 * 
	 * @since 1.7 moved from functions.php
	 * 
	 * @param string $lang whose flag
	 * @return string
	 */
	static function getLanguageFlag($lang) {
	...
	static function getLanguageText($loc = NULL, $separator = NULL) {
		$locale_subdomains = i18n::getLanguageSubdomains();
		if (is_null($loc)) {
			$text = @$locale_subdomains[sanitize(zp_getCookie('zpcms_locale'))];
		} else {
			$text = @$locale_subdomains[$loc];
	 ...
	 * @return string
	 */
	static function getLanguageDisplayName($locale) {
		if (class_exists('Locale')) {
			return Locale::getDisplayName($locale);
		} else {
			$languages = self::getLanguageArray();
