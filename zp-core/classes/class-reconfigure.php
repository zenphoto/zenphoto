 ...
 * 
 * @since 1.7 Reworked as class replacing functions-reconfigure.php
 * 
...
class reconfigure {
	 ...
	 * 
	 * @since 1.7 Moved from functions-reconfigure.php
	 * 
	 * @param int $mandatory 0-3 0 means signature change where 1 means setup must autorun assuming a fresh install
	 */
	static function reconfigureAction($mandatory) {
		list($diff, $needs) = self::checkSignature($mandatory);
	 ...
	 * @return type
	 */
	static function checkSignature($auto) {
	 ...
	 * @return string
	 */
	static function signatureChange($tab = NULL, $subtab = NULL) {
		list($diff, $needs) = self::checkSignature(false);
		self::printReconfigureNote($diff, $needs, 0);
		return $tab;
	}
	 ...
	 * @since 1.7 Moved from functions-reconfigure.php
	 */
	static function addReconfigureNote() {
		if (function_exists('zp_register_filter')) {
			filter::registerFilter('admin_head', 'reconfigure::printCSS');
			filter::registerFilter('admin_note', 'reconfigure::signatureChange');
			filter::registerFilter('theme_head', 'reconfigure::printCSS');
			filter::registerFilter('theme_body_open', 'reconfigure::signatureChange');
		}
	}
	 ...
	 */
	static function printReconfigureNote($diff, $needs, $mandatory) {
		$notice = self::getReconfigureNote($diff, $needs, $mandatory);
		if (!zp_loggedin()) {
			debuglog("Reconfignote hidden on frontend as not loggedin");
			self::debuglogReconfigureNote($notice);
			return;
		}
		?>
	 ...
	 * @return array
	 */
	static function getReconfigureNote($diff, $needs, $mandatory) {
	 ...
	 * 
	 */
	static function printReconfigureError($mandatory) {
	 ...
	 */
	static function debuglogReconfigureNote($notice) {
	 ...
	 * 
	 */
	static function ignoreSetupRunRequest() {
		if (isset($_GET['ignore_setup']) && zp_loggedin(ADMIN_RIGHTS)) {
			...
			/**
			 * Invoked when setup creates the security log.
			 *
			 * @param bool true for succes, false for failed
			 * @param string The action taken
			 * @param string Additional log information
			 *
			 * @package filters
			 * @subpackage logs
			 */
	 ...
	 * @return array
	 */
	static function isSetupProtected() {
		if (file_exists(SERVERPATH . '/' . ZENFOLDER . '/setup/')) {
	 ...
	 * 
	 */
	static function unprotectSetupFiles() {
		$found = self::isSetupProtected();
		if ($found) {
	 ...
	 * 
	 */
	static function protectSetupFiles() {
		chdir(SERVERPATH . '/' . ZENFOLDER . '/setup/');
		$list = safe_glob('*.php');
		if (!empty($list)) {
			$rslt = array();
