 ...
 * 
 * @since 1.7 Changed to class imageProcessor replacing functions-image.php
 * 
 * @package core\classes\helbers
 *
 */
class imageProcessor {

	/**
	 * Prints an image error directly or wittin the debuglog
	 * 
	 * @since 1.7 moved from functions-image.php
	 * 
	 ...
	 */
	static function imageError($status_text, $errormessage, $errorimg = 'err-imagegeneral.png', $image = '', $album = '', $newfilename = '') {
	 ...
	 */
	static function imageDebug($album, $image, $args, $imgfile) {
	 ...
	 * @return array
	 */
	static function propSizes($size, $width, $height, $w, $h, $thumb, $image_use_side, $dim) {
		$hprop = round(($h / $w) * $dim);
		$wprop = round(($w / $h) * $dim);
		if ($size) {
	 ...
	 * @param $value
	 */
	static function iptcMakeTag($rec, $data, $value) {
		$length = strlen($value);
	 ...
	 */
	static function cacheImage($newfilename, $imgfile, $args, $allow_watermark = false, $theme = '', $album = '') {
		...
		try {
			...
			if (!file_exists($imgfile) || !is_readable($imgfile)) {
				self::imageError('404 Not Found', sprintf(gettext('Image %s not found or is unreadable.'), filesystemToInternal($imgfile)), 'err-imagenotfound.png');
			}
			$rotate = false;
			if ($_zp_graphics->imageCanRotate()) {
				$rotate = self::getImageRotation($imgfile);
			}
			$s = getSuffix($imgfile);
			...
			if (!$im) {
				self::imageError('404 Not Found', sprintf(gettext('Image %s not renderable (imageGet).'), filesystemToInternal($imgfile)), 'err-failimage.png', $imgfile, $album, $newfilename);
			}
			if ($rotate) {
				if (DEBUG_IMAGE) {
				...
				if (!$im) {
					self::imageError('404 Not Found', sprintf(gettext('Image %s not rotatable.'), filesystemToInternal($imgfile)), 'err-failimage.png', $imgfile, $album, $newfilename);
				}
			}
			...
			if (!empty($size)) {
				...
				self::imageError('404 Not Found', sprintf(gettext('Unknown error processing %s! Please report to the developers at <a href="https://www.zenphoto.org/">www.zenphoto.org</a>'), filesystemToInternal($imgfile)), 'err-imagegeneral.png', $imgfile, $album, $newfilename);
			}
			...
			if ($passedWM) {
				if ($passedWM != NO_WATERMARK) {
					...
					if (!file_exists($watermark_image)) {
						$watermark_image = SERVERPATH . '/' . ZENFOLDER . '/images_default_sidecars/imageDefault.png';
					}
				}
			} else {
				if ($allow_watermark) {
			...
			if ($crop) {
				...
				if (!$_zp_graphics->resampleImage($newim, $im, 0, 0, intval($cx), intval($cy), $neww, $newh, $cw, $ch)) {
					self::imageError('404 Not Found', sprintf(gettext('Image %s not renderable (resample).'), filesystemToInternal($imgfile)), 'err-failimage.png', $imgfile, $album, $newfilename);
				}
			} else {
				...
				if (($thumb && $sharpenthumbs) || (!$thumb && $sharpenimages)) {
					if (!$_zp_graphics->imageUnsharpMask($newim, getOption('sharpen_amount'), getOption('sharpen_radius'), getOption('sharpen_threshold'))) {
						self::imageError('404 Not Found', sprintf(gettext('Image %s not renderable (unsharp).'), filesystemToInternal($imgfile)), 'err-failimage.png', $imgfile, $album, $newfilename);
					}
				}
			}
			...
			$newim = self::addWatermark($newim, $watermark_image, $imgfile);
			...
			if ($_zp_graphics->imageOutput($newim, getSuffix($newfile), $newfile, $quality)) { //	successful save of cached image
				if (getOption('EmbedIPTC') && getSuffix($newfilename) == 'jpg' && GRAPHICS_LIBRARY != 'Imagick') { // the embed function works only with JPEG images
					...
					require_once(SERVERPATH . '/' . ZENFOLDER . '/definitions-core.php'); //	it is ok to increase memory footprint now since the image processing is complete
					$iptc = array(
				...
				self::imageError('404 Not Found', sprintf(gettext('cacheImage: failed to create %s'), $newfile), 'err-failimage.png', $imgfile, $album, $newfilename);
			}
			@chmod($newfile, FILE_MOD);
			...
			self::imageError('404 Not Found', sprintf(gettext('cacheImage(%1$s) exception: %2$s'), $newfilename, $e->getMessage()), 'err-failimage.png', $imgfile, $album, $newfilename);
			return false;
		}
		clearstatcache();
		return true;
	 ...
	 * @return array|false
	 */
	static function getImageRotation($imgfile) {
		global $_zp_db;
		$rotation = false;
	 ...
	 */
	static function addWatermark($newim, $watermark_image, $imgfile = null) {
		global $_zp_graphics;
		if ($watermark_image) {
			$watermark = $_zp_graphics->imageGet($watermark_image);
			if (!$watermark) {
				self::imageError('404 Not Found', sprintf(gettext('Watermark %s not renderable.'), $watermark_image), 'err-failimage.png');
			}
			...
			if ($nw != $watermark_width || $nh != $watermark_height) {
				...
				if (!$_zp_graphics->resampleImage($watermark_new, $watermark, 0, 0, 0, 0, $nw, $nh, $watermark_width, $watermark_height)) {
					self::imageError('404 Not Found', sprintf(gettext('Watermark %s not resizeable.'), $watermark_image), 'err-failimage.png');
				}
			}
			...
			if (!$_zp_graphics->copyCanvas($newim, $watermark_new, $dest_x, $dest_y, 0, 0, $nw, $nh)) {
				self::imageError('404 Not Found', sprintf(gettext('Image %s not renderable (copycanvas).'), filesystemToInternal($imgfile)), 'err-failimage.png', $imgfile, '', '');
			}
			 ...
			 */
			if (GRAPHICS_LIBRARY != 'GD' || (GRAPHICS_LIBRARY == 'GD' && imageProcessor::isGDImage($watermark_new))) {
	 ...
	 * @return boolean
	 */
	static function isGDImage($image) {
