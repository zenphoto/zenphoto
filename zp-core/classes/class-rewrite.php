 ...
 * 
 * @since 1.7 Reworked as class replacing functions-rewrite.php
 *
...
class rewrite {
	 ...
	 * 
	 * @since 1.7 moved from functions-rewrite.php
	 * 
	 * @global array $_zp_conf_vars
	 * @global type $_zp_rewritten
	 */
	static function rewriteHandler() {
		...
		list($definitions, $rules) = static::getRules();
	...
	/**
	 * Sets the definition of all registered rewrite constants.
	 * 
	 * @since 1.7 wraps former procedural code
	 * 
	 * @global array $_zp_conf_vars
	 */
	static function setRewriteConstants() {
		global $_zp_conf_vars;
		$_definitions = array();
		foreach ($_zp_conf_vars['special_pages'] as $definition) {
			if (@$definition['define'] && !defined($definition['define'])) { // failsafe
