			...
			if (isset($_GET['album']) && !isset($_GET['massedit'])) {
				...
				if ($subtab == 'albuminfo') {
					...
					<div id="tab_albuminfo" class="tabbox">
						...
						<form class="dirtylistening" name="albumedit1" id="form_albumedit" autocomplete="off" action="?page=edit&amp;action=save<?php echo "&amp;album=" . pathurlencode($album->name); ?>"	method="post">
					...
					if (count($subalbums) > 0) {
						...
						<div id="tab_subalbuminfo" class="tabbox">
							...
							?>
							<form class="dirtylistening" action="?page=edit&amp;album=<?php echo pathurlencode($album->name); ?>&amp;action=savesubalbumorder&amp;tab=subalbuminfo" method="post" name="sortableListForm" id="sortableListForm" onsubmit="return confirmAction();">
					...
					<div id="tab_imageinfo" class="tabbox">
						...
						if ($allimagecount) {
							?>
							<form class="dirtylistening" name="albumedit2"	id="form_imageedit" action="?page=edit&amp;action=save<?php echo "&amp;album=" . html_encode(pathurlencode($album->name)); ?>"	method="post" autocomplete="off">
								<?php XSRFToken('albumedit'); ?>
								<input type="hidden" name="album"	value="<?php echo $album->name; ?>" />
				...
				<form class="dirtylistening" name="albumedit" id="form_albumedit-multi" autocomplete="off"	action="?page=edit&amp;action=save<?php echo $albumdir ?>" method="POST">
					<?php XSRFToken('albumedit'); ?>
				...
				if (count($albums) > 0) {
					...
					?>
					<form class="dirtylistening" action="?page=edit&amp;action=savealbumorder" method="post" name="sortableListForm" id="sortableListForm" onsubmit="return confir
																										mAction();">
									<?php XSRFToken('savealbumorder'); ?>
						<p class="buttons">
							<?php
