			...
			if (is_AdminEditPage('newsarticle')) {
				...
				if (is_AdminEditPage('newscategory')) {
					...
					if ($result->loaded || $result->transient) {
						if ($result->transient) {
							?>
							<form class="dirtylistening" method="post" name="addnews" action="admin-edit.php?<?php echo $admintype; ?>&amp;save">
								<?php
								XSRFToken('save');
							} else {
								?>
								<form class="dirtylistening" method="post" name="update" id="form_zenpageitemedit" action="admin-edit.php?<?php echo $admintype; ?>&amp;update<?php echo $page; ?>">
									<?php
									XSRFToken('update');
								}
								?>
