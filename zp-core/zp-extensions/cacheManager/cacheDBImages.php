								...
								if (file_exists(getAlbumFolder() . $base . '.' . $suffix)) {
									...
									if (strpos($uri, 'i.php?') !== false) {
										...
										<a href="<?php echo html_encode($uri); ?>&amp;debug" title="<?php echo $title; ?>">
											<?php
											if (isset($args[10])) {
												echo '<img class="iplink" src="' . html_encode(pathurlencode($uri)) . '&cached=' . rand() . '" height="15" width="15" alt="x" />' . "\n";
											} else {
												echo '<img class="iplink" src="' . html_encode(pathurlencode($uri)) . '&cached=' . rand() . '" height="20" width="20" alt="X" />' . "\n";
											}
											?>
										</a>
										<?php
